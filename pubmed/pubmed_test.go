...
func TestNumberOfSets(t *testing.T) {
	...
	ps.ResultCount, err = pubmed.ResultsCount(mockResponseJSON["count"])
...
func TestRealSearch(t *testing.T) {
	...
	fmt.Println(ps.ResultCount)
	fmt.Println(ps.QueryKey)
	fmt.Println(ps.WebEnv)
}
//
...
//	ps := pubmed.NewSearch(query)
//	ps.Name = "Cardiology"
//	xps, err := ps.QueryArticles("29735362", "29730991")
